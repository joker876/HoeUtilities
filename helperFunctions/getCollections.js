            ...
            try {
                ...
                Promise.all(promises).then(values => {
                    ...
                    if (heldItem.getString('id').match(/HOE_CANE/)) {
                        if (global.hoeutils.collections.cane.API != profile.collection.SUGAR_CANE || firstLoad) {
                            global.hoeutils.collections.cane.counter = counter
                        }
                    } else if (heldItem.getString('id').match(/HOE_POTATO/)) {
                        if (global.hoeutils.collections.potato.API != profile.collection.POTATO_ITEM || firstLoad) {
                            global.hoeutils.collections.potato.counter = counter
                        }
                    } else if (heldItem.getString('id').match(/HOE_CARROT/)) {
                        if (global.hoeutils.collections.carrot.API != profile.collection.CARROT_ITEM || firstLoad) {
                            global.hoeutils.collections.carrot.counter = counter
                        }
                    } else if (heldItem.getString('id').match(/HOE_WHEAT/)) {
                        if (global.hoeutils.collections.wheat.API != profile.collection.WHEAT || firstLoad) {
                            global.hoeutils.collections.wheat.counter = counter
                        }
                    } else if (heldItem.getString('id').match(/HOE_WARTS/)) {
                        if (global.hoeutils.collections.wart.API != profile.collection.NETHER_STALK || firstLoad) {
                ...
                global.hoeutils.isCollectionError = 1200;
            }
        })
    } catch (error) {
        global.hoeutils.isCollectionError = 1200;
